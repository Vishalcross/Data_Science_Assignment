...
def error_in_predictions(loc_PM10,loc_PM25,data_2017,predictions):
    '''
    ...
    for j in keyset:
        ...
        for i in range(count,len(data_2017),24):
            error_10 += (data_2017[i][loc_PM10] - pred[count2][0])**2
            error_25 += (data_2017[i][loc_PM25] - pred[count2][1])**2
            count2 += 1
        error_10 /= len(data_2017)
        error_25 /= len(data_2017)
        average_PM10 += error_10
        average_PM25 += error_25
        error_PM10[j] = error_10
        error_PM25[j] = error_25
        count += 1
    average_PM10 /= len(keyset)
    average_PM25 /= len(keyset)

    return average_PM10, average_PM25
...
def main():
    ...
    z = []
    while k < 10:
        print("Iteration no. %d"%k)
        nearest_neighbors = 2 ** k
        predictions = k_nearest_neighbours(nearest_neighbors,loc_PM10,loc_PM25,data_2016,data_2017,0)
        errors_PM10, errors_PM25 = error_in_predictions(loc_PM10,loc_PM25,data_2017,predictions)
        k += 1
        x.append(k)
        y.append(errors_PM10)
        z.append(errors_PM25)
    plot1,=plt.plot(x,y)
    plot2,=plt.plot(x,z)
    plt.xlabel('log2_of_no_of_nearest_neighbors')
    plt.ylabel('Mean Squared Error')
    plt.legend([plot1,plot2],['PM10','PM25'])
    plt.title('Recursive Window')
    plt.savefig('recursive_window.png',dpi=300)
    ...
    plt.savefig('rolling_window.png',dpi=300)
    ...
    plt.ylabel('Mean Squared Error')
    plt.legend([plot1,plot2],['PM10','PM25'])
    plt.title('Normalised Recursive Window')
    plt.savefig('normalised_recursive.png',dpi=300)
    ...
    plt.ylabel('Mean Squared Error')
    plt.legend([plot1,plot2],['PM10','PM25'])
    plt.title('Normalised Rolling Window')
    plt.savefig('normalised_rolling.png',dpi=300)
